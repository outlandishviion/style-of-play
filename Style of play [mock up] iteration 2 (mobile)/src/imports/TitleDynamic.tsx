...
function Frame3({ minValue }: { minValue: number }) {
  return (
    <div className="content-stretch flex flex-col font-['Open_Sans:Regular',sans-serif] font-normal items-start relative shrink-0 text-white w-[40px]">
...
function Frame4({ maxValue }: { maxValue: number }) {
  return (
    <div className="content-stretch flex flex-col font-['Open_Sans:Regular',sans-serif] font-normal items-end relative shrink-0 text-nowrap text-white w-[40px]">
      ...
      <p className="leading-[normal] relative shrink-0 text-[14px] whitespace-pre" style={{ fontVariationSettings: "'wdth' 100" }}>
        {maxValue >= 97 ? `>97%` : `${maxValue}%`}
      </p>
    </div>
  );
}

function Frame2({ minValue, maxValue }: { minValue: number; maxValue: number }) {
  return (
    <div className="box-border content-stretch flex gap-[16px] h-full items-center pb-0 pt-[4px] px-0 relative shrink-0">
      ...
      <Frame4 maxValue={maxValue} />
    </div>
  );
}

function TextIcon({ minValue, maxValue }: { minValue: number; maxValue: number }) {
  return (
    <div className="basis-0 content-stretch flex gap-[4px] grow items-center min-h-px min-w-px relative shrink-0" data-name="Text/Icon">
      ...
      <div className="flex flex-row items-center self-stretch">
        <Frame2 minValue={minValue} maxValue={maxValue} />
      </div>
    </div>
  );
}

export default function TitleDynamic({ minValue, maxValue }: TitleProps) {
  return (
    <div className="box-border content-stretch flex gap-[8px] items-start pb-0 pt-[4px] px-0 relative size-full" data-name="title">
      <TextIcon minValue={minValue} maxValue={maxValue} />
    </div>
  );
}